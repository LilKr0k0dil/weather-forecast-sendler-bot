...
def get_gmt_offset(query):
    ...
    print(lat, '  ',lng,'   fwegwgwegwegwegwegdsvsdsfhwfhwfiuwuhhufehwehhuwfuhfwhehehuuhwwu')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Привет! Я очень хочу помочь сделать вашу жизнь проще. Чтобы начать работу, укажите пожалуйста сперва ваше имя а затем"
                                    "и город о котором вы хотите получать уведомления с информацией о погоде?"
                                    " Спасибо!")
    await update.message.reply_text("Укажите ваше имя. Например Евгений")
    return 2
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if '/' in update.message.text:
        ...
        if not cursor.execute("SELECT id FROM Users WHERE id=?", (current_data['user_id'],)).fetchone():
            cursor.execute("""INSERT INTO Users (id, name) VALUES (?, ?)""",
                            (current_data['user_id'], current_data['name']))
        else:
            print(current_data['name'], current_data['user_id'],'  qqqq')
            cursor.execute("UPDATE Users SET name=? WHERE id=?",
                            (current_data['name'],current_data['user_id']))
        conn.commit()
        await update.message.reply_text("Для какого населенного пункта и из какой страны вы хотите получать информацию о погоде? Укажите ближайший узнаваемый город.(Ввод через запятую. Например Москва, Россия)")
        return 3
...
async def get_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ', ' not in current_data['location']:
        ...
        if not cursor.fetchone():
            cursor.execute("""INSERT INTO Users (id, location, timezone) VALUES (?, ?, ?)""",
                            (current_data['user_id'], current_data['location'], current_data['timezone']))
        else:
        ...
        await update.message.reply_text("Каковы ваши предпочтения в показываемой информации о погоде?", reply_markup=reply_markup)
        return 4
...
async def get_w_preferences(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "all":
        ...
        cursor.execute("SELECT id FROM Users WHERE id=?", (current_data['user_id'],))
        if not cursor.fetchone():
            cursor.execute("""INSERT INTO Users (id, w_preferences) VALUES (?, ?)""",
                            (current_data['user_id'], current_data['w_preferences']))
        else:
            cursor.execute("""UPDATE Users SET w_preferences=? WHERE id=?""",
                            (current_data['w_preferences'], current_data['user_id']))
        conn.commit()

        await context.bot.send_message(context._chat_id,"Когда вы хотите получать уведомления и оповещения? Я буду писать вам каждый день (предположительно до того, как вы проснетесь), поэтому, пожалуйста, выберите "
                                "одно время (часовой пояс - тот, в котором вы находитесь). Введите свой ответ в формате 'одно время в 24-часовой системе "
...
async def get_a_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ((len(current_data['a_schedule']) == 5) and ('00' <= current_data['a_schedule'][:2] <= '24') and (current_data['a_schedule'][2] == ':') and ('00' <= current_data['a_schedule'][3:] <= '59')):
        await update.message.reply_text("Пожалуйста, введите время в соответствии с инструкциями. Часы - это 00-24, посередине ставится двоеточие (:), а минуты - 00-59. "
                                        "Другой пример: '13:05'. Попробуйте еще раз!")
        return 5
    ...
    cursor.execute("SELECT id FROM Users WHERE id=?", (current_data['user_id'],))
    if not cursor.fetchone():
        cursor.execute("""INSERT INTO Users (gmt_schedule) VALUES (?)""",
                        (current_data['gmt_schedule']))
    else:
    ...
    await update.message.reply_text("Спасибо, что предоставили мне всю необходимую информацию. Я обязательно сообщу и предупрежу вас!\n"
                                    "Тем временем вы можете изменить свою информацию для меня, нажав на кнопки клавиатуры "
                                    "(например, если вы путешествуете, вы можете изменить местоположение).", reply_markup=markup2)
    return ConversationHandler.END


async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text('Действие отменено.', reply_markup=markup2)
    try:
    ...
    if 'asked' not in context.user_data:
        ...
        if '/' in update.message.text and update.message.text != '/cancel':
            ...
            cursor.execute("""UPDATE Users SET name=? WHERE id=?""",
                        (current_data['name'], current_data['user_id']))
            conn.commit()
            await update.message.reply_text(f"Здравствуйте, {current_data['name']}!", reply_markup=markup2)
            del context.user_data['asked']
            return ConversationHandler.END
...
async def update_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'asked' not in context.user_data:
        context.user_data['asked'] = True
        await update.message.reply_text("По какому новому адресу вы будете получать уведомления? Пожалуйста, укажите ближайший узнаваемый город.(Москва, Россия)", reply_markup=markup3)
        return 1
    else:
        current_data['location'] = update.message.text
        pattern = r'^([а-яА-ЯёЁa-zA-Z]+),\s*([а-яА-ЯёЁa-zA-Z]+)$'
    ...
    # Проверка сообщения по регулярному выражению
        ...
        if ', ' not in current_data['location']:
            ...
            gmt_offset = get_gmt_offset(current_data['location'])[0]
            if gmt_offset == "City not found":
                await update.message.reply_text("Мне не удалось найти ваш город. Пожалуйста, попробуйте ввести более узнаваемый город поблизости(Москва, Россия)")
                return 1
            current_data['timezone'] = gmt_offset
            cursor.execute("""UPDATE Users SET location=?, timezone=? WHERE id=?""",
                            (current_data['location'], current_data['timezone'], current_data['user_id']))
            conn.commit()
            await update.message.reply_text(f"Теперь ваше местоположение установлено на {current_data['location']}!", reply_markup=markup2)
            del context.user_data['asked']
            return ConversationHandler.END
...
async def get_w_preferences2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "all":
        ...
        await query.edit_message_text('Предпочтения успешно изменены')
        #global current_w_preferences_list
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(context._chat_id, f"Ваши данные:\nИмя: {name}\nГород: {location}\nПредпочтения по погоде: {pref}\nВремя отправки: {gmt_to_local(time, timezone)}\n")
    
async def update_a_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'asked' not in context.user_data:
        context.user_data['asked'] = True
        await update.message.reply_text("Когда вы хотите получать уведомления и оповещения? Я буду писать вам каждый день (предположительно до того, как вы проснетесь), поэтому, пожалуйста, выберите "
                                    "одно время (часовой пояс - тот, в котором вы находитесь). Введите свой ответ в формате 'одно время в 24-часовой системе "
        ...
        if not ((len(current_data['a_schedule']) == 5) and ('00' <= current_data['a_schedule'][:2] <= '24') and (current_data['a_schedule'][2] == ':') and ('00' <= current_data['a_schedule'][3:] <= '59')):
            ...
            await update.message.reply_text(f"Теперь ваше время для оповещений установлено на {current_data['a_schedule'].strip()}!", reply_markup=markup2)
            del context.user_data['asked']
            return ConversationHandler.END
...
def fetch_weather(current_location, current_w_preferences, current_name):
    ...
    personalised_data = f'Здравствуйте, {current_name}! Информация о прогнозе погоды для {current_location} сегодня ({today_str})!\n' +  ' \n'
    # Adding the alert
    ...
    if data['days'][0]['severerisk'] < 30:
        ...
        level = 'высокий'
    personalised_data += f'Кроме того, существует {level} уровень риска конвективных бурь (например, гроз, града и торнадо).\n' + ' \n'
    ...
    if current_w_preferences_list[3] == '1':
        personalised_data += f"Осадки: Жидкие Осадки {data['days'][0]['precip']}mm, Вероятность {data['days'][0]['precipprob']}%, Ненулевые осадки для {data['days'][0]['precipcover']} часов, и "
